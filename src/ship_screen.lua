...
setNextTextAlign(Lx,AlignH_Left,AlignV_Middle) addText(Lx,fB,"◄ NAVIGATOR v2.1 ►",8,C/2)
if nName~="" then
...
if lo=="importarch" then
  if not navdatabank then SetStatus("navdatabank not linked — share the Arch databank with the Navigator",8); DrawScreen(); return end
  local raw=navdatabank.getStringValue("SavedLocations")
  if not raw or raw=="" then SetStatus("No SavedLocations key found in navdatabank",8); DrawScreen(); return end
  local ok,locs=pcall(json.decode,raw)
  if not ok or type(locs)~="table" then SetStatus("Failed to parse SavedLocations",8); DrawScreen(); return end
  local imported=0
...
if lo=="importsaga" then
  if not navdatabank then SetStatus("navdatabank not linked — share the SAGA databank with the Navigator",8); DrawScreen(); return end
  local raw=navdatabank.getStringValue("SagaRoutes")
  if not raw or raw=="" then SetStatus("No SagaRoutes key found in navdatabank",8); DrawScreen(); return end
  local fn=load("return "..raw)
  if not fn then SetStatus("Could not parse SagaRoutes data",8); DrawScreen(); return end
  local ok,sagaRoutes=pcall(fn)
  if not ok or type(sagaRoutes)~="table" then SetStatus("Failed to read SagaRoutes",8); DrawScreen(); return end
  local imported=0
